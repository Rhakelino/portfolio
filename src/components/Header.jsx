...
const Header = () => {
  const [cvUrl, setCvUrl] = useState("/files/CV_ALMALIKUL_RHAKELINO_2025.pdf");

  useEffect(() => {
    // Fetch public URL from supabase
    const { data } = supabase.storage.from("documents").getPublicUrl("cv.pdf");
    if (data?.publicUrl) {
      // Use the database URL. Browser caching can be an issue with upsert, 
      // adding a timestamp query param ensures users always get the freshest CV.
      setCvUrl(`${data.publicUrl}?t=${new Date().getTime()}`);
    }
  }, []);
  ...
  return (
    <div className="relative min-h-[100svh] flex flex-col items-center justify-center overflow-hidden">
      ...
      <motion.div
        ...
        className="absolute w-[300px] h-[300px] md:w-[500px] md:h-[500px] rounded-full -z-10 bg-[radial-gradient(circle,hsl(var(--primary))_0%,transparent_70%)]"
      />
